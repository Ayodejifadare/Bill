...
export function json(data: unknown, init: ResponseInit = {}): Response {
  const headers = mergeHeaders(init.headers, { 'Content-Type': 'application/json; charset=utf-8' });
...
function resolveCorsOrigin(originConf: CorsOptions['origin'], origin: string | null): string | null {
  if (!originConf) return null;
  if (originConf === '*') return '*';
  if (typeof originConf === 'string') return originConf;
  if (originConf instanceof RegExp) return origin && originConf.test(origin) ? origin : null;
  if (typeof originConf === 'function') return originConf(origin) ? (origin || '*') : null;
  return null;
}

export function withCors(handler: ServerlessHandler, options: CorsOptions = {}): ServerlessHandler {
  const allowMethods = (options.methods || ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS']).join(', ');
  const allowHeaders = (options.headers || ['Content-Type', 'Authorization', 'x-user-id']).join(', ');
  ...
  return async (req, ctx) => {
    ...
    const response = res instanceof Response
      ? res
      : json(res?.body ?? null, { status: res?.status ?? 200, headers: res?.headers });
...
export function withErrorHandling(handler: ServerlessHandler): ServerlessHandler {
  return async (req, ctx) => {
    try {
...
export function createServerlessFunction(handler: ServerlessHandler, cors?: CorsOptions) {
  const base: ServerlessHandler = async (req, _ctx) => handler(req, { json, text });
