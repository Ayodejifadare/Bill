...
const CONTACT_ERROR_MESSAGES: Record<ContactErrorCode, string> = {
  ...
  "invalid-file": "Invalid file. Please use a supported contacts file.",
...
function prepareContactForMatching(
  contact: Contact,
  region: string,
  index: number,
): PreparedContact {
  ...
  const displayName =
    contact.displayName || contact.name || `Contact ${index + 1}`;
...
class ContactsAPI {
  ...
  async checkPermissionStatus(): Promise<ContactPermissionStatus> {
    try {
      ...
      if (
        ...
        "cordova" in window &&
        (window as any).cordova?.plugins?.contacts
      ) {
  ...
  async requestPermission(): Promise<ContactPermissionStatus> {
    try {
      ...
      if (
        ...
        try {
          const permission =
            await window.Capacitor.Plugins.Contacts.requestPermissions();
          const status =
            permission?.contacts ||
      ...
      if ("showOpenFilePicker" in window && !this.isInCrossOriginContext()) {
        try {
          ...
          if (error instanceof DOMException && error.name === "SecurityError") {
            console.warn(
              "File picker not available in cross-origin context, falling back to file input",
            );
  ...
  async matchContacts(contacts: Contact[]): Promise<MatchedContact[]> {
    ...
    const prepared = contacts.map((contact, index) =>
      prepareContactForMatching(contact, region, index),
    );
    ...
    const buildResults = (entries: any[]): MatchedContact[] => {
      ...
      const matchedContacts = entries.map((entry, idx): MatchedContact => {
        ...
        const status =
          entry.status === "existing_user" ? "existing_user" : "not_on_app";
        ...
        return {
          ...
          mutualFriends:
            typeof entry.mutualFriends === "number"
              ? entry.mutualFriends
              : undefined,
          avatar: entry.avatar,
        };
...
export class WhatsAppAPI {
  ...
  static createWhatsAppInviteLink(
    phone: string,
  ...
  static isWhatsAppAvailable(): boolean {
    // WhatsApp Web works on desktop and mobile
