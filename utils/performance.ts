  ...
  endNavigation(route: string) {
    ...
    if (entries.length > 0) {
      ...
      if (navigationTime > 1000) {
        console.warn(
          `Slow navigation to ${route}: ${navigationTime.toFixed(2)}ms`,
        );
      }
    }
  }
  ...
  private setupMemoryMonitoring() {
    if ("memory" in performance) {
      ...
      setInterval(() => {
        ...
        if (memoryUsage > 50) {
          // Alert if memory usage exceeds 50MB
        ...
        for (const entry of list.getEntries()) {
          if (entry.entryType === "resource" && entry.name.includes(".js")) {
            ...
            if (bundleSize > 500000) {
              // Alert for bundles larger than 500KB
              console.warn(
                `Large bundle detected: ${entry.name} (${(bundleSize / 1024).toFixed(2)}KB)`,
              );
            }
          }
        }
...
export const trackNavigation = {
  start: (route: string) =>
    PerformanceMonitor.getInstance().startNavigation(route),
