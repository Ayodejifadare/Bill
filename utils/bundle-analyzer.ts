...
export class BundleAnalyzer {
  ...
  private isLazyChunk(url: string): boolean {
    ...
    return url.includes('lazy') || url.includes('chunk') || url.match(/\d+\.[a-f0-9]+\.js$/);
  }
  ...
  private generateRecommendations(analysis: BundleAnalysis): string[] {
    ...
    if (totalJSSize > 1000000) { // 1MB
      recommendations.push('⚠️ Total JavaScript bundle size exceeds 1MB. Consider code splitting.');
    }

    // Large individual chunks
    jsChunks.forEach(chunk => {
      if (chunk.size > 500000) { // 500KB
        recommendations.push(`⚠️ Large chunk detected: ${chunk.name} (${this.formatSize(chunk.size)})`);
      }
    });

    // Lazy loading suggestions
    const nonLazyChunks = jsChunks.filter(chunk => !chunk.isLazy && chunk.size > 100000);
    if (nonLazyChunks.length > 0) {
      recommendations.push('💡 Consider lazy loading these large components: ' + 
        nonLazyChunks.map(chunk => chunk.name).join(', '));
    }

    // Asset optimization
    const assetChunks = analysis.chunks.filter(chunk => chunk.type === 'asset');
    const largeAssets = assetChunks.filter(chunk => chunk.size > 200000);
    if (largeAssets.length > 0) {
      recommendations.push('🖼️ Large assets detected. Consider image optimization: ' + 
    ...
    if (analysis.totalSize > 2000000) { // 2MB
      recommendations.push('🚨 Total bundle size exceeds 2MB performance budget!');
    }
  ...
  generateReport(analysis: BundleAnalysis): string {
    ...
    analysis.chunks.forEach((chunk, index) => {
      ...
      report.push(`${index + 1}. ${icon} ${chunk.name}: ${this.formatSize(chunk.size)}${lazy}`);
    });
