...
describe('authService', () => {
  ...
  describe('verifyOTP', () => {
    it('posts normalized numbers with the otp and returns auth data', async () => {
      const response = { token: 'token-123', user: { id: 'user-1', name: 'Alice' } }
      apiClientMock.mockResolvedValueOnce(response)

      const result = await authService.verifyOTP('0049 123 456', '654321', 'Alice', true)
      ...
      expect(result).toEqual({ success: true, token: 'token-123', user: { id: 'user-1', name: 'Alice' } })
    })
