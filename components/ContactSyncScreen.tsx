...
export function ContactSyncScreen({ onNavigate }: ContactSyncScreenProps) {
  ...
  const [syncMethod, setSyncMethod] = useState<"contacts" | "file" | "demo">(
    "contacts",
  );
  ...
  const performContactSync = async () => {
    try {
      ...
      setTimeout(() => {
        ...
        const existingUsers = matched.filter(
          (c) => c.status === "existing_user",
        ).length;
        const inviteableContacts = matched.filter(
          (c) => c.status === "not_on_app",
        ).length;

        if (existingUsers > 0) {
          toast.success(
            `Found ${existingUsers} friend${existingUsers !== 1 ? "s" : ""} on Biltip!`,
          );
        }

        if (inviteableContacts > 0) {
          toast.success(
            `${inviteableContacts} contact${inviteableContacts !== 1 ? "s" : ""} can be invited to join!`,
          );
        }
  ...
  const handleFileImport = async () => {
    try {
      ...
      if (
        ...
        showContactError(
          "File picker not available in this environment. Please try the upload option.",
        );
  ...
  const handleDemoMode = async () => {
    ...
    try {
      ...
      toast.success(
        `Demo complete! Found ${existingUsers} friend${existingUsers !== 1 ? "s" : ""} on Biltip.`,
      );
    } catch (error) {
  ...
  const denyPermission = () => {
    ...
    toast.info(
      "Contact sync skipped. You can add friends manually or try again later.",
    );
