...
vi.mock("./UserProfileContext", () => ({
  useUserProfile: () => ({ appSettings: { region: "US" } }),
}));
...
describe("GroupAccountScreen", () => {
  it("switches default account", async () => {
    ...
    await waitFor(() =>
      expect(fetchMock).toHaveBeenCalledWith(
        "/api/groups/1/accounts/2",
        expect.objectContaining({ method: "PUT" }),
      ),
    );

    await waitFor(() => {
      const headerContainer = screen.getByText("Account2").closest("div")!
  ...
  it("deletes an account", async () => {
    ...
    await waitFor(() =>
      expect(fetchMock).toHaveBeenCalledWith("/api/groups/1/accounts/1", {
        method: "DELETE",
      }),
    );
    await waitFor(() =>
