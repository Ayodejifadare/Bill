...
import { MessageCircle, FileText, Image, X, Loader2 } from "lucide-react";
...
export function ShareSheet({
  ...
  const triggerNativeShare = async (data: {
    title: string;
    text: string;
    files?: File[];
  }) => {
    ...
    if (canNativeShare()) {
      ...
      try {
        ...
        console.warn(
          "Text sharing failed, falling back to clipboard:",
          shareError,
        );
  ...
  const handleWhatsAppShare = async () => {
    ...
    try {
      ...
      try {
        const result = await fallbackToManualCopy(
          whatsappText,
          "whatsapp_failed",
        );
        ...
        if (result.fallback === "clipboard") {
          if (result.method === "modern") {
            ...
            toast.success(
              "WhatsApp failed to open. Text copied using legacy method.",
            );
          }
        ...
        toast.error(
          "Unable to share or copy. Please try again or copy manually.",
        );
      }
    } finally {
  ...
  const handlePDFShare = async () => {
    ...
    try {
      ...
      try {
        ...
        if (result?.fallback === "clipboard") {
          if (result.reason === "insecure_context") {
            toast.success(
              "Content copied to clipboard (sharing requires HTTPS)",
            );
          } else {
        ...
        if (error.name === "AbortError") {
          ...
          if (result.fallback === "clipboard") {
            if (result.method === "modern") {
              toast.success(
                "Permission denied. PDF content copied to clipboard.",
              );
            } else if (result.method === "legacy") {
              toast.success(
                "PDF sharing failed. Content copied using legacy method.",
              );
            }
          } else if (result.fallback === "manual") {
            toast.success(
              "PDF sharing failed. Content prepared for manual copying.",
            );
          }
        }
      }
      ...
      try {
        ...
        toast.error(
          "Unable to share PDF or copy text. Please try again or copy manually.",
        );
      }
    } finally {
      setIsProcessing(null);
  ...
  const handleImageShare = async () => {
    ...
    try {
            ...
            if (blob) {
              try {
                ...
                if (error.name === "AbortError") {
                  ...
                  if (result.fallback === "clipboard") {
                    if (result.method === "modern") {
                      toast.success(
                        "Permission denied. Image content copied to clipboard.",
                      );
                    } else if (result.method === "legacy") {
                      toast.success(
                        "Image sharing failed. Content copied using legacy method.",
                      );
                    }
                  } else if (result.fallback === "manual") {
                    toast.success(
                      "Image sharing failed. Content prepared for manual copying.",
                    );
                  }
                }
              }
              ...
              try {
                ...
                if (result.fallback === "clipboard") {
                  if (result.method === "modern") {
                    toast.success(
                      "Image generation failed. Content copied to clipboard.",
                    );
                  } else if (result.method === "legacy") {
                    toast.success(
                      "Image generation failed. Content copied using legacy method.",
                    );
                  }
                } else if (result.fallback === "manual") {
                  toast.success(
                    "Image generation failed. Content prepared for manual copying.",
                  );
                }
              } catch (fallbackError) {
      ...
      try {
        ...
        toast.error(
          "Unable to share image or copy text. Please try again or copy manually.",
        );
      }
    } finally {
      setIsProcessing(null);
