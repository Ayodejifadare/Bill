...
export function RecurringPaymentsScreen({
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener(
        "recurringPaymentsUpdated",
        fetchRecurringPayments,
      );
  }, [fetchRecurringPayments]);

  const createPayment = useCallback(
    async (data: Partial<RecurringPayment>) => {
      try {
  ...
  const togglePaymentStatus = async (paymentId: string) => {
    ...
    toast.success(
      `Recurring payment ${newStatus === "active" ? "resumed" : "paused"}`,
    );
  };
  ...
  const deletePayment = async () => {
    if (selectedPayment) {
      try {
        await apiClient(`/recurring-payments/${selectedPayment}`, {
          method: "DELETE",
        });
  ...
  const completedPayments = recurringPayments.filter(
              ...
              <div className="flex items-center justify-between mb-1">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem
                      onClick={() =>
                        onNavigate("setup-recurring-payment", {
              ...
              <div className="flex items-center justify-between mt-2">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {payment.completedPayments} of{" "}
                    {payment.totalPayments || "∞"} payments
                  </p>
                </div>
              </div>
            </div>
              ...
              <Card>
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-sm text-muted-foreground mb-4">
                    Set up recurring payments to automate your regular bills and
                    subscriptions.
                  </p>
                  <Button onClick={() => onNavigate("setup-recurring-payment")}>
                    <Plus className="h-4 w-4 mr-2" />
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this recurring payment? This
              action cannot be undone and will stop all future automatic
              payments.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
