...
describe("AddGroupMemberScreen", () => {
  it("handles permission denied", async () => {
    (contactsAPI.checkPermissionStatus as any).mockResolvedValue({
      granted: false,
      denied: false,
    });
    (contactsAPI.requestPermission as any).mockResolvedValue({
      granted: false,
      denied: true,
      prompt: false,
    });
  ...
  it("syncs contacts successfully", async () => {
    ...
    (contactsAPI.getContacts as any).mockResolvedValue([
      { id: "1", name: "Alice", phoneNumbers: ["123"], emails: [] },
    ]);
  ...
  it("submits invites", async () => {
    ...
    (toast.error as any).mockClear();
    fireEvent.change(screen.getByPlaceholderText("Enter their name"), {
