...
describe('MemberInviteScreen', () => {
  it('creates invite link', async () => {
    ...
    const fetchMock = vi.fn((url: string, options?: any) => {
      if (url === '/api/groups/g1/invites') {
        return Promise.resolve({ ok: true, json: async () => ({ invites: [] }) } as Response);
      }
      if (url === '/api/groups/g1/invite-links' && (!options || options.method === undefined)) {
        return Promise.resolve({ ok: true, json: async () => ({ links: [] }) } as Response);
      }
      if (url === '/api/groups/g1/invite-links' && options?.method === 'POST') {
        return Promise.resolve({ ok: true, json: async () => ({ link: newLink }) } as Response);
      }
  ...
  it('resends invite', async () => {
    ...
    const updatedInvite = { ...invite, attempts: 2, status: 'delivered' as const, lastAttempt: '2024-01-16T00:00:00Z' };

    const fetchMock = vi.fn((url: string, options?: any) => {
      if (url === '/api/groups/g1/invites') {
        return Promise.resolve({ ok: true, json: async () => ({ invites: [invite] }) } as Response);
      }
      if (url === '/api/groups/g1/invite-links') {
        return Promise.resolve({ ok: true, json: async () => ({ links: [] }) } as Response);
      }
      if (url === '/api/groups/g1/invites/i1/resend' && options?.method === 'POST') {
        return Promise.resolve({ ok: true, json: async () => ({ invite: updatedInvite }) } as Response);
      }
      return Promise.reject(new Error('unknown url'));
    });
    vi.spyOn(global, 'fetch').mockImplementation(fetchMock as any);
