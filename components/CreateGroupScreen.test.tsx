...
describe('CreateGroupScreen', () => {
  it('validates required fields', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('e.g., Work Squad, Roommates, Travel Buddies'), { target: { value: 'Trip' } });
    fireEvent.click(screen.getByText('Next'));
  ...
  it('creates group successfully', async () => {
    ...
    render(<CreateGroupScreen onNavigate={onNavigate} initialSelectedFriendIds={['1']} />);

    fireEvent.change(screen.getByPlaceholderText('e.g., Work Squad, Roommates, Travel Buddies'), { target: { value: 'Trip' } });
    fireEvent.change(screen.getByPlaceholderText('What will you use this group for?'), { target: { value: 'Desc' } });
    fireEvent.click(screen.getByText('Next'));
    await waitFor(() => expect(screen.getAllByRole('button', { name: 'Create Group' }).length).toBeGreaterThan(0));
