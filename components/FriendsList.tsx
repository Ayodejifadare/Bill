...
export function FriendsList({ onNavigate }: FriendsListProps) {
  ...
  async function loadFriends() {
    ...
    try {
      ...
      friendsData = (friendRes.friends || []).map(
        (f: {
          id: string;
          name: string;
          username?: string;
          ...
          direction:
            f.direction ?? (f.status === "pending" ? "incoming" : undefined),
        }),
      );
    } catch (err) {
        ...
        <div className="grid grid-cols-2 gap-4">
          <Card className="p-4 text-center">
            <p className="text-2xl text-success">
              {formatCurrencyForRegion(appSettings.region, summary.owedToUser)}
            </p>
            <p className="text-sm text-muted-foreground">You're owed</p>
          </Card>
          <Card className="p-4 text-center">
            <p className="text-2xl text-destructive">
              {formatCurrencyForRegion(appSettings.region, summary.userOwes)}
            </p>
        ...
        {error ? (
          ...
          <>
                        ...
                        <div className="flex space-x-2">
                          {friend.direction === "outgoing" ? (
                            <Button
                              ...
                              onClick={() =>
                                friend.requestId &&
                                handleCancelRequest(friend.requestId)
                              }
                            >
                              Cancel
                            </Button>
                            ...
                            <>
                              <Button
                                ...
                                onClick={() =>
                                  friend.requestId &&
                                  handleDeclineRequest(friend.requestId)
                                }
                              >
                                Decline
                              </Button>
                              <Button
                                ...
                                onClick={() =>
                                  friend.requestId &&
                                  handleAcceptRequest(friend.requestId)
                                }
                              >
                                Accept
                              </Button>
            ...
            {activeFriends.length < 3 && searchQuery.trim() === "" && (
              <Card
                className="p-4 border-dashed border-2 border-muted hover:border-primary/50 transition-colors cursor-pointer"
