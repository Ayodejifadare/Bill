...
export function SpendingInsightsScreen({
  ...
  const spentIsSmallUp =
    spentDelta >= neutralThreshold && spentDelta <= warningThreshold;
  ...
  const splitsIsSmallDown =
    splitsDelta <= -neutralThreshold && splitsDelta >= -warningThreshold;
    ...
    const fetchInsights = async () => {
      ...
      try {
        const data = await apiClient(
          `/spending-insights?period=${encodeURIComponent(selectedPeriod)}`,
        );
      ...
      <div className="px-4 py-6 space-y-6 pb-8">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Card className="p-4 min-h-[120px] flex flex-col justify-between">
            <div className="flex items-center justify-between mb-3">
              ...
              <Badge
                variant="outline"
                className={`${splitsBadgeClass} text-xs`}
              >
            ...
            <div className="flex-1 flex flex-col justify-end">
              <p className="text-xl sm:text-2xl font-bold">
                {spendingData.overview?.splitsCount ?? 0}
              </p>
          ...
          <div className="space-y-3">
            {spendingData.insights.map((insight: any, index: number) => {
              ...
              return (
                <Card
                  key={index}
                  className={`p-4 min-h-[60px] ${getInsightBgColor(insight.type)}`}
                >
                  <div className="flex items-start space-x-3">
                    <div
                      className={`p-2 rounded-full ${getInsightBgColor(insight.type)} flex-shrink-0`}
                    >
                      <IconComponent
                        className={`h-5 w-5 ${getInsightColor(insight.type)}`}
                      />
                    </div>
        ...
        <Tabs defaultValue="categories" className="w-full">
          <TabsList className="grid w-full grid-cols-3 min-h-[48px]">
            <TabsTrigger value="categories" className="text-sm min-h-[44px]">
              Categories
            </TabsTrigger>
            <TabsTrigger value="trends" className="text-sm min-h-[44px]">
              Trends
            </TabsTrigger>
            <TabsTrigger value="goals" className="text-sm min-h-[44px]">
              Goals
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="categories" className="space-y-6 mt-6">
            ...
            <Card className="p-4 sm:p-6">
              ...
              {spendingData.currentMonth.categories.length === 0 ? (
                ...
                <div className="space-y-4">
                  {spendingData.currentMonth.categories.map(
                    (category, index) => (
                      <div
                        key={index}
                        className="space-y-3 min-h-[44px] flex flex-col justify-center"
                      >
              ...
              {spendingData.goals.length === 0 ? (
                ...
                spendingData.goals.map((goal, index) => {
                  ...
                  return (
                    <Card
                      key={index}
                      className={`p-4 min-h-[120px] ${isOverBudget ? "border-destructive" : ""}`}
                    >
                      <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex justify-between text-xs text-muted-foreground">
                            ...
                            <span>
                              {fmt(Math.max(goal.budget - goal.spent, 0))}{" "}
                              remaining
                            </span>
                          </div>
                        </div>
