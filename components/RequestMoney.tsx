...
export function RequestMoney({ onNavigate, prefillData }: RequestMoneyProps) {
  ...
  const loadGroups = useCallback(async () => {
    try {
      ...
      const groupsData: Group[] = (data.groups || []).map(
        (g: {
          id: string;
          name: string;
  ...
  useEffect(() => {
    ...
    async function loadMethods() {
      try {
        ...
        const defaultMethod =
          mapped.find((m) => m.isDefault) || mapped[0] || null;
        ...
        const fallback: PaymentMethod[] = (
          userProfile?.linkedBankAccounts || []
        ).map((acc) => ({
          id: acc.id,
          type: "bank",
  ...
  useEffect(() => {
    ...
    const fid =
      (prefillData as any).friendId || (prefillData as any).recipientId;
  ...
  const addGroupMembers = (group: Group) => {
    const newMembers = group.members.filter(
      (member) => !selectedFriends.some((f) => f.id === member.id),
    );
  ...
  const handleSendRequest = async () => {
    ...
    try {
      ...
      await createRequest({
        ...
        recurringDayOfWeek:
          isRecurring && recurringFrequency === "weekly"
            ? (dayOfWeekMap[recurringDayOfWeek as keyof typeof dayOfWeekMap] ??
              0)
            : undefined,
      });
      ...
      if (isRecurring) {
        toast.success(
          `Recurring money request created! This will be sent to ${selectedFriends.length} friend${selectedFriends.length > 1 ? "s" : ""} ${recurringFrequency}. Payment details have been included.`,
        );
      } else {
        toast.success(
          `Money request sent to ${selectedFriends.length} friend${selectedFriends.length > 1 ? "s" : ""}! Payment details have been included.`,
        );
      }
  ...
  const shareRequestDetails = async () => {
    ...
    const formattedAmount = formatCurrencyForRegion(
      appSettings.region,
      parseFloat(amount),
    );
    const lines: string[] = [];
  ...
  const getNextPaymentDate = () => {
    ...
    if (recurringFrequency === "weekly") {
      ...
      const targetDay =
        daysOfWeek[recurringDayOfWeek as keyof typeof daysOfWeek];
  ...
  const getRecurringDescription = () => {
    if (recurringFrequency === "weekly") {
      const dayName =
        recurringDayOfWeek.charAt(0).toUpperCase() +
        recurringDayOfWeek.slice(1);
          ...
          <>
            ...
            <div className="space-y-3">
              ...
              {showGroupSelection && (
                <div className="space-y-2">
                  <p className="text-xs text-muted-foreground">
                    Select a group to add all members:
                  </p>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 w-full">
                            <div
                              className={`w-8 h-8 rounded-full ${group.color} flex items-center justify-center`}
                            >
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {formatCurrencyForRegion(
                    appSettings.region,
                    parseFloat(amount) * selectedFriends.length,
                  )}
                </span>
              </div>
              <div className="flex justify-between">
            ...
            {isRecurring && (
              <div className="pl-4 border-l-2 border-border space-y-3">
                  ...
                  {recurringFrequency === "quarterly" && (
                    <div>
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="1">
                            1st of every 3rd month
                          </SelectItem>
                          <SelectItem value="15">
                            15th of every 3rd month
                          </SelectItem>
                          <SelectItem value="last">
                ...
                <div className="p-3 bg-muted/50 rounded-lg space-y-2">
                  <p className="text-sm">
                    <strong>Note:</strong> This will automatically send the same
                    request {getRecurringDescription()} to the selected friends.
                    You'll receive notifications when requests are sent.
                  </p>
