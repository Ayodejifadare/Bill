...
export function GroupMembersScreen({ groupId, onNavigate }: GroupMembersScreenProps) {
  ...
  const allFilteredResults = [
    ...filteredMembers,
    ...(activeFilter === 'all' || activeFilter === 'pending' ? filteredPendingInvites.map(invite => ({ 
      ...invite, 
  ...
  const getStatusBadge = (member: any) => {
    if (member.status === 'pending') {
      return <Badge variant="outline" className="text-xs text-warning">Pending</Badge>;
    }
    ...
    return (
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem onClick={() => onNavigate('friend-profile', { friendId: member.id })}>
            View Profile
          </DropdownMenuItem>
  ...
  return (
    <div>
      <ScreenHeader
        ...
        rightAction={
          canManageMembers ? (
            <Button size="sm" onClick={() => onNavigate('add-group-member', { groupId })}>
      ...
      <div className="p-4 space-y-6 pb-20">
        ...
        <FilterTabs
          ...
          onTabChange={(tab) => setActiveFilter(tab as 'all' | 'active' | 'pending')}
        />
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Card className="p-4 text-center">
            ...
            <p className="text-2xl font-medium">
              {activeMembers.filter(m => m.lastActive.includes('minutes') || m.lastActive.includes('hour')).length}
            </p>
        ...
        {allFilteredResults.length > 0 ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {member.status === 'pending' ? (
                        ...
                        <>
                          <p className="text-xs text-muted-foreground">
                            Joined {formatDate(member.joinedAt)} • Last active {member.lastActive}
                          </p>
                            ...
                            <div className="flex gap-4 mt-2">
                              ...
                              {member.totalOwed > 0 && (
                                <div className="text-xs">
                                  ...
                                  <span className="font-medium text-warning">${member.totalOwed.toFixed(2)}</span>
                                </div>
                              )}
                            </div>
                          )}
          ...
          <EmptyState
            ...
            description={searchQuery ? "Try adjusting your search" : "This group doesn't have any members yet"}
            actionLabel={canManageMembers ? "Add Members" : undefined}
            onAction={canManageMembers ? () => onNavigate('add-group-member', { groupId }) : undefined}
          />
        )}
