...
describe("classifyStatus", () => {
  ...
  it("classifies past dates as overdue", () => {
    expect(classifyStatus(new Date(now.getTime() - 24 * 60 * 60 * 1000))).toBe(
      "overdue",
    );
  });
  ...
  it("classifies dates within 7 days as due_soon", () => {
    expect(
      classifyStatus(new Date(now.getTime() + 3 * 24 * 60 * 60 * 1000)),
    ).toBe("due_soon");
  });

  it("classifies dates beyond 7 days as upcoming", () => {
    expect(
      classifyStatus(new Date(now.getTime() + 10 * 24 * 60 * 60 * 1000)),
    ).toBe("upcoming");
  });
});
