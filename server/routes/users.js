...
function inferLookupType(value) {
  ...
  if (numericCandidate.length >= 7 && PHONE_CANDIDATE_PATTERN.test(trimmed))
    return "phone";
...
const useS3 =
  (process.env.AVATAR_STORAGE || "").toLowerCase() === "s3" ||
  ...
  try {
    ...
    if (lookupType === "email") {
      ...
      const variants = new Set(
        buildPhoneVariants(rawIdentifier, currentUser?.region || "US"),
      );
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const notif = await req.prisma.notificationPreference.findUnique({
      where: { userId: req.params.id },
    });
    ...
    res.json({
      user: {
        ...rest,
        preferences: { ...(preferenceSettings || {}), notificationSettings },
      },
    });
  } catch (error) {
...
router.get("/:id/stats", async (req, res) => {
  try {
    ...
    const [sentAgg, receivedAgg, splitsCount, friendsCount] =
  ...
  try {
    ...
    if (useS3) {
      try {
        ...
        const prefix = (process.env.S3_PREFIX || "uploads/avatars").replace(
          /^\/+|\/+$/g,
          "",
        );
        ...
        return res
          ...
          .json({
            error:
              "Avatar upload failed. Ensure AWS SDK is installed or disable S3 storage.",
          });
      }
    } else {
      // Local disk fallback
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (preferences?.biometrics !== undefined)
        data.biometricEnabled = preferences.biometrics;
      ...
      if (preferences !== undefined) {
        ...
        if (preferences.notifications !== undefined)
          updates.push = { enabled: preferences.notifications };
        if (preferences.emailAlerts !== undefined)
          updates.email = { enabled: preferences.emailAlerts };
        if (preferences.whatsappAlerts !== undefined)
          updates.whatsapp = { enabled: preferences.whatsappAlerts };
        if (Object.keys(updates).length > 0) {
          notificationSettings = await updateNotificationPreference(
            req.prisma,
            req.params.id,
            updates,
          );
    ...
    if (user.preferenceSettings) {
      if (typeof user.preferenceSettings === "string") {
        try {
          ...
          console.warn(
            "Failed to parse stored preferenceSettings JSON:",
            error,
          );
          preferences = {};
        }
  ...
  try {
    ...
    if (privacy) {
      if (privacy.twoFactorAuth !== undefined)
        data.twoFactorEnabled = !!privacy.twoFactorAuth;
      if (privacy.biometricAuth !== undefined)
        data.biometricEnabled = !!privacy.biometricAuth;
    }
    if (preferences) {
      try {
    ...
    if (notifications) {
      notificationsOut = await updateNotificationPreference(
        req.prisma,
        req.params.id,
        notifications,
        false,
    ...
    if (updatedUser.preferenceSettings) {
      if (typeof updatedUser.preferenceSettings === "string") {
        try {
          ...
          console.warn(
            "Failed to parse updated preferenceSettings JSON:",
            error,
          );
          preferencesOut = {};
        }
