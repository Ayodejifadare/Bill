...
describe("Friend routes", () => {
  ...
  it("lists active friends along with incoming and outgoing pending requests", async () => {
    ...
    const incoming = await prisma.friendRequest.create({
      data: { senderId: "u3", receiverId: "u1" },
    });
    const outgoing = await prisma.friendRequest.create({
      data: { senderId: "u1", receiverId: "u4" },
    });
  ...
  it("returns 400 for empty search query", async () => {
    await prisma.user.create({
      data: { id: "e1", email: "e1@example.com", name: "User 1" },
    });
  ...
  it("includes last transaction information for active friends", async () => {
    await prisma.user.create({
      data: { id: "a1", email: "a1@example.com", name: "A1" },
    });
    await prisma.user.create({
      data: { id: "a2", email: "a2@example.com", name: "A2" },
    });
    await prisma.user.create({
      data: { id: "a3", email: "a3@example.com", name: "A3" },
    });
    await prisma.user.create({
      data: { id: "a4", email: "a4@example.com", name: "A4" },
    });
  ...
  it("returns summary of amounts owed with friends", async () => {
    ...
    await prisma.user.create({
      data: { id: "u2", email: "u2@example.com", name: "User 2" },
    });
    await prisma.user.create({
      data: { id: "u3", email: "u3@example.com", name: "User 3" },
    });
    await prisma.user.create({
      data: { id: "u4", email: "u4@example.com", name: "User 4" },
    });
  ...
  it("returns incoming and outgoing friend requests with metadata", async () => {
    ...
    const incoming = await prisma.friendRequest.create({
      data: { senderId: "a2", receiverId: "a1" },
    });
    const outgoing = await prisma.friendRequest.create({
      data: { senderId: "a1", receiverId: "a3" },
    });
  ...
  it("cancels friend requests", async () => {
    ...
    await prisma.user.create({
      data: { id: "u5", email: "u5@example.com", name: "User 5" },
    });
    const fr = await prisma.friendRequest.create({
      data: { senderId: "u4", receiverId: "u5" },
    });
