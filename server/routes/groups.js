  ...
  try {
    ...
    for (const bs of billSplits) {
      ...
      if (memberSpentMap.has(bs.createdBy)) {
        memberSpentMap.set(
          bs.createdBy,
          memberSpentMap.get(bs.createdBy) + Number(bs.totalAmount || 0),
        );
      }
      for (const p of bs.participants) {
        ...
        if (memberSpentMap.has(p.userId)) {
          memberSpentMap.set(
            p.userId,
            memberSpentMap.get(p.userId) + Number(p.amount || 0),
          );
        }
        ...
        if (p.userId !== bs.createdBy && p.isPaid === false) {
          if (memberBalanceMap.has(bs.createdBy)) {
            memberBalanceMap.set(
              bs.createdBy,
              memberBalanceMap.get(bs.createdBy) + Number(p.amount || 0),
            );
          }
          if (memberBalanceMap.has(p.userId)) {
            memberBalanceMap.set(
              p.userId,
              memberBalanceMap.get(p.userId) - Number(p.amount || 0),
            );
          }
        }
      }
    ...
    // Collapse multiple rows from the same bill split to avoid
    // one entry per participant for the current user.
    const seen = new Set();
    const deduped = [];
    for (const t of transactions) {
      const key = t.billSplitId || t.id;
      if (seen.has(key)) continue;
      seen.add(key);
      deduped.push(t);
      if (deduped.length >= pageSize + 1) break;
    }
  ...
  try {
    ...
    const formatted = await Promise.all(
      contacts.map(async (c) => {
        ...
        const membership = await req.prisma.groupMember.findUnique({
          where: {
            groupId_userId: { groupId: req.params.groupId, userId: user.id },
          },
        });
