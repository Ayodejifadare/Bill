...
router.post('/initiate-transfer', authenticate, async (req, res) => {
  try {
    const { billSplitId, recipientName, bankName, accountNumber, amount, narration } = req.body || {}

    if (!billSplitId || !recipientName || !bankName || !accountNumber || !amount) {
    ...
    const isParticipant = billSplit.createdBy === req.user.id || billSplit.participants.some(p => p.userId === req.user.id)
    if (!isParticipant) {
      return res.status(403).json({ error: 'Not authorized for this bill split' })
    }
