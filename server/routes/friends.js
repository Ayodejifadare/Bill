...
// Send friend request
router.post("/request", [body("receiverId").notEmpty()], async (req, res) => {
  try {
...
router.get("/summary", async (req, res) => {
  try {
    ...
    const owedToUser =
      (owedAgg._sum.amount || 0) + (unpaidAsCreator._sum.amount || 0);
    const userOwes =
      (owesAgg._sum.amount || 0) + (unpaidAsParticipant._sum.amount || 0);
...
router.get("/:friendId", async (req, res) => {
  try {
    ...
    if (balance > 0) currentBalance = { amount: balance, type: "owed" };
      ...
      currentBalance = { amount: Math.abs(balance), type: "owes" };
