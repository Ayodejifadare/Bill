...
async function main() {
  ...
  while (true) {
    ...
    for (const u of users) {
      ...
      const needsUpdate = u.region !== region || u.currency !== currency;

      if (needsUpdate) {
        console.log(
          `[backfill] user=${u.id} phone=${phone} ${u.region}/${u.currency} -> ${region}/${currency}`,
        );
        if (!DRY_RUN) {
  ...
  console.log(
    `[backfill] Done. inspected=${inspected} updated=${updated} dryRun=${DRY_RUN}`,
  );
}
