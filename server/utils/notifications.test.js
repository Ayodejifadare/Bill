...
const { createNotification, broadcastUnreadCount } = await import(
  "./notifications.js"
);
...
describe("notifications utils", () => {
  ...
  describe("createNotification", () => {
    ...
    it("includes optional fields and broadcasts unread count", async () => {
      ...
      expect(sendEvent).toHaveBeenCalledWith(baseData.recipientId, {
        unread: 5,
      });
    });
  ...
  describe("broadcastUnreadCount", () => {
    ...
    it("logs and swallows errors when counting fails", async () => {
      ...
      await expect(
        broadcastUnreadCount(prisma, "user-2"),
      ).resolves.toBeUndefined();

      expect(consoleSpy).toHaveBeenCalledWith(
        "Broadcast unread count error:",
        error,
      );
