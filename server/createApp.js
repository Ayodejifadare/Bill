...
export default function createApp({ enableSchedulers = true } = {}) {
  ...
  const configuredFrontend =
    normalizeOrigin(process.env.FRONTEND_URL) || defaultFrontend;
  const additionalFrontendOrigins = (process.env.FRONTEND_URLS || "")
    .split(",")
    .map((origin) => normalizeOrigin(origin.trim()))
    .filter(Boolean);
  const allowedProdOrigins = Array.from(
    new Set([configuredFrontend, ...additionalFrontendOrigins]),
  );
  const devOrigins = Array.from(
    new Set([
      ...allowedProdOrigins,
      normalizeOrigin("http://localhost:4000"),
  ...
  const rateLimitEnabled =
    (process.env.RATE_LIMIT_ENABLED ??
      (process.env.NODE_ENV !== "development" ? "true" : "false")) !== "false";
  if (rateLimitEnabled) {
    ...
    const limiter = rateLimit({
      ...
      skip: (req) =>
        req.method === "OPTIONS" || req.path === "/notifications/stream",
    });
    app.use("/api", limiter);
  }
